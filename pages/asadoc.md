...
1.  Plotting energy bands require the use of a symmetry-line file. Rather than create one, it can be found in the _lm_{: style="color: green"} repository. First, navigate in to the build directory
    ...
    <div onclick="elm = document.getElementById('box1'); if(elm.style.display == 'none') elm.style.display = 'block'; else elm.style.display = 'none';"><button type="button" class="btn btn-default btn-xs btn-block">The symmetry-line file should look as such (click to show).</button></div>
